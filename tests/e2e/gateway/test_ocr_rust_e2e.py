...
INVOICE_FIXTURE = Path(__file__).parent / "fixtures" / "invoice_three_page.pdf"
...
def _three_page_pdf_data_uri() -> str:
    if not INVOICE_FIXTURE.is_file():
        raise FileNotFoundError(
            f"missing invoice fixture {INVOICE_FIXTURE}; regenerate it with "
            "tests/e2e/gateway/fixtures/generate_invoice_fixture.py"
        )
    encoded = base64.b64encode(INVOICE_FIXTURE.read_bytes()).decode("ascii")
