...
use common_utils::{
    azure_operation_polling_timeout, classify_reqwest_error, convert_document_url_to_data_uri,
    has_header, ocr_provider_config, poll_document_intelligence, string_headers,
    truncate_error_body, upload_reducto_document,
};
...
pub async fn ocr(request: OcrRequest<'_>) -> CoreResult<Value> {
        ...
        && status.as_u16() == 202
    {
        ...
        let deadline = request
            .timeout
            .unwrap_or_else(azure_operation_polling_timeout);
        let response_json =
            poll_document_intelligence(&operation_url, &url, &upstream_headers, deadline).await?;
        return Ok(config
...
mod tests {
    ...
    async fn document_intelligence_polling_times_out_when_never_succeeds() {
        ...
        server.abort();
        assert_eq!(error, CoreError::Timeout, "{error:?}");
    }
